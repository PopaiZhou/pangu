...
public class DepotHeadDAO extends BaseDAO<DepotHead> implements DepotHeadIDAO {
    ...
    public void findAreaReport(PageUtil pageUtil, String beginTime, String endTime, String state) throws JshException {
        ...
        queryString.append("d.templateName,b.MUnit,b.OperNumber,b.UnitPrice,b.AllPrice,s.customerNo,dh.state,dh.city FROM jsh_depothead dh");
        ...
        if("全国".equals(state)){
            queryString.append(" WHERE dh.OperTime >='").append(beginTime).append("' and dh.OperTime<='").append(endTime).append("' and dh.SendStatus=1");
        }else {
    ...
    public void findAreaReportTemplate(PageUtil pageUtil, String beginTime, String endTime, String state) throws JshException {
        ...
        if("全国".equals(state)){
            queryString.append("WHERE b.OperTime >='").append(beginTime).append("' and b.OperTime<='").append(endTime).append("' and b.SendStatus='1'").append(" GROUP BY templateName");
        }else{
            queryString.append("WHERE b.state = '").append(state).append("' and b.OperTime >='").append(beginTime).append("' and b.OperTime<='").append(endTime).append("' and b.SendStatus='1'").append(" GROUP BY templateName");
        }
    ...
    public void findSupplierStatementAccount(PageUtil pageUtil, String beginTime, String endTime, Long organId) throws JshException {
        StringBuilder queryString = new StringBuilder();
        queryString.append("SELECT dh.Number,date_format(dh.OperTime, '%Y-%m-%d') AS oTime,c.productName,d.templateName,a.MUnit,a.OperNumber,a.TaxUnitPrice,s.customerNo,s.customerName,dh.state,dh.city FROM jsh_depotitem a ");

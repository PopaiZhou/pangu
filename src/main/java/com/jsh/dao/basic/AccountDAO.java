...
public class AccountDAO extends BaseDAO<Account> implements AccountIDAO {
    ...
    public void findAccountInOutList(PageUtil<Account> pageUtil, Long accountId) throws JshException {
        ...
        //主表出入库涉及的账户
        queryString.append("select dh.Number,concat(dh.SubType,dh.Type) as newType,s.customerName,dh.TotalPrice,date_format(dh.OperTime,'%Y-%m-%d %H:%i:%S') as oTime,'' as AList,'' as AMList " +
                " from jsh_depothead dh inner join jsh_customer s on dh.OrganId = s.id where 1=1 ");
        if (accountId != null && !accountId.equals("")) {
            queryString.append(" and dh.AccountId='" + accountId + "' ");
        }
        //主表收入和支出涉及的账户
        queryString.append("UNION ALL " +
                "select ah.BillNo,ah.Type as newType,s.customerName,ah.TotalPrice,date_format(ah.BillTime,'%Y-%m-%d %H:%i:%S') as oTime,'' as AList,'' as AMList " +
                "  from jsh_accounthead ah inner join jsh_customer s on ah.OrganId=s.id where 1=1 ");
        if (accountId != null && !accountId.equals("")) {
            queryString.append(" and ah.AccountId='" + accountId + "' ");
        }
        //明细中涉及的账户（收款,付款,收预付款）
        queryString.append("UNION ALL " +
                "select ah.BillNo,ah.Type as newType,s.customerName,ai.EachAmount,date_format(ah.BillTime,'%Y-%m-%d %H:%i:%S') as oTime,'' as AList,'' as AMList " +
                " from jsh_accounthead ah inner join jsh_customer s on ah.OrganId=s.id " +
        ...
        //主表中转出的账户
        queryString.append("UNION ALL " +
                "select ah.BillNo,ah.Type as newType, '' as sName,ah.TotalPrice,date_format(ah.BillTime,'%Y-%m-%d %H:%i:%S') as oTime,'' as AList,'' as AMList " +
        ...
        //多账户的情况
        queryString.append("UNION ALL " +
                "select dh.Number,concat(dh.SubType,dh.Type) as newType,s.customerName,dh.TotalPrice,date_format(dh.OperTime,'%Y-%m-%d %H:%i:%S') as oTime,dh.AccountIdList,dh.AccountMoneyList" +
                " from jsh_depothead dh inner join jsh_customer s on dh.OrganId = s.id where 1=1 ");
        if (accountId != null && !accountId.equals("")) {
